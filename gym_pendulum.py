...
if __name__ == "__main__":
    ...
    N_BATCH = 1
    LQR_ITER = 5
    ACTION_LOW = -2.0 # this should be -1
    ACTION_HIGH = 2.0 # this should be +1

    class PendulumDynamics(torch.nn.Module): #non-linear dynamics of the Pendulum, for some reason implemented as a torch.nn.Module
    ...
    goal_weights = torch.tensor((1., 0.1))  # nx --> these are the weights given to each state in the cost-function
    goal_state = torch.tensor((0., 0.))  # nx --> this is the goal state or the reference
    ctrl_penalty = 0.001
    ...
    for i in range(run_iter):
        ...
        # recreate controller using updated u_init (kind of wasteful right?) --> in here the optimization step is happening, let's study this
        ctrl = mpc.MPC(nx, nu, TIMESTEPS, u_lower=ACTION_LOW, u_upper=ACTION_HIGH, lqr_iter=LQR_ITER, exit_unconverged=False, 
                       eps=1e-2, n_batch=N_BATCH, backprop=False, verbose=0, u_init=u_init, grad_method=mpc.GradMethods.AUTO_DIFF)
