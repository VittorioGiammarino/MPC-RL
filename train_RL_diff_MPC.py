...
class Workspace(object):
    def __init__(self, cfg):
        ...
        # actor input --> 2*nx + u
        # actor output --> (nx+nu)
        ctrl_horizon_space = (self.train_env.action_space.shape[0]*self.cfg.T,)
        ctrl_dim = self.train_env.action_space.shape
        self.net_action_space = (ctrl_dim[0]+self.train_env.observation_space_dim[0],) # 4 weights parameters for the Q-matrix + 1 for the control penalty
        ...
        while eval_until_episode(episode):
            ...
            while not done:
                ...
                with torch.no_grad(), utils.eval_mode(self.agent):
                    params = self.agent.act(observation, self.global_step, eval_mode=True)
                ...
                # compute action based on current state, dynamics, and cost
                nominal_states, nominal_u, nominal_objs = self.mpc_agent(state, QP_cost_actor, EnvMismatchedDynamics(self.eval_env), u_init)
                u = nominal_u[0]  # take first planned action
                u_init = torch.cat((nominal_u[1:], torch.zeros(1, self.N_BATCH, self.nu)), dim=0)
                assert u.ndim == 2 and u.shape[0] == self.eval_env.action_space.shape[0]
    ...
    def train(self):
        # predicates
        train_until_step = utils.Until(self.cfg.num_train_states, 1)
        seed_until_step = utils.Until(self.cfg.num_seed_states, 1)
        eval_every_step = utils.Every(self.cfg.eval_every_states, 1)
        ...
        while train_until_step(self.global_step):
            ...
            # sample action
            with torch.no_grad(), utils.eval_mode(self.agent):
                params = self.agent.act(observation, self.global_step, eval_mode=False)
                QP_cost_actor = self.calculate_QP_cost(params)

            # try to update the agent
            if not seed_until_step(self.global_step):
                metrics = self.agent.update(self.replay_buffer, 
                                            EnvMismatchedDynamics(self.eval_env).to(self.device), 
                                            self.mpc_agent.to(self.device), 
                                            self.cfg.T, 
                                            self.goal_state, 
                                            self.global_step)
            ...
            # dynamic step
            next_obs, reward, done, _ = self.train_env.step_dynamic(u.detach().cpu().numpy()[0])

            next_state = self.train_env.state.copy()
            done = float(done)
            done_no_max = 0 if episode_step + 1 == self.train_env.n_steps else done
            episode_reward += reward
            nominal_u = nominal_u.detach().cpu().numpy().squeeze()
            self.replay_buffer.add(state, observation, nominal_u, reward, next_state, next_obs, done, done_no_max)
...
@hydra.main(config_path='config_folder', config_name='config_ac_diff_MPC')
def main(cfg):
    from train_RL_diff_MPC import Workspace as W
    workspace = W(cfg)
    workspace.eval_MPC()
    workspace.train()
