...
class CartPoleBTEnv(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        n_steps = env_config.get('n_steps', 200)
        controller_type = env_config.get('controller_type', 'lqr')
        ...
        self.masscart_high = 8.0
        self.masscart_low = 3.0
        self.masspole_high = 3.5
        self.masspole_low = 0.5
        self.length_high = 3.0
        self.length_low = 1.0
        self.friction_high = 4.0
        self.friction_low = 0.1
        ...
        self.n_steps = n_steps # max number of steps per episode
        self.time_step = 0
        self.kinematics_integrator = 'RK45' #runge kutta integrator
    ...
    @property
    def action_space_dim(self): # this cons
        return (4,)

    @property
    def full_observation_space_dim(self):
        #4 for the actual agent state (s), 4 for the goal state or reference (s_g), 1 for the actuation (u)
        return (9,)
    ...
    def cost_function(self, state):
        ...
        the goal state.
        """
        return ((state[0] - self.goal_state[0])**2 + (angle_normalize(state[2]) - self.goal_state[2])**2)

    def output(self, state, u):
        system_state = self.output_matrix.dot(state)
        control_action = np.array([u], dtype='float32')
        return {"state": system_state, "reference":self.goal_state, "control_action": control_action}

    def ODE_solver(self, t, x, u):
    ...
    def step_dynamic(self, u):
        ...
        reward = -self.cost_function(self.state) # - (u)**2
        ...
        for v in output.values():
            observation = np.concatenate((observation, np.array(v)))

        assert observation.shape[0] == self.full_observation_space_dim[0]

        x, x_dot, theta, theta_dot = self.state
        info = {"x": x, "x_dot":x_dot, "theta":theta, "theta_dot":theta_dot, 'masspole': self.masspole, 
                'masscart':self.masscart, 'length':self.length, 'friction':self.friction, "control_action":[u]}

        return observation, reward, done, info

    def controller(self, masspole, masscart, length, g, friction, s):

        A, B, C, D = cartpend_ss(masspole, masscart, length, g, friction, s)

        if self.controller_type == 'place':
            p = [-0.5, -1, -2, -3]
            # Compute gain matrix using pole placement
            gain = control.place(A, B, p)

        elif self.controller_type == 'lqr':
            Q = np.eye(4)
            R = 1
            gain, _, _ = control.lqr(A, B, Q, R)

        return gain

    def unsquash(self, value, high, low):
        remapped_action = ((value+1.0)/2.0)*(high-low)+low
        return np.clip(remapped_action, low, high)

    def step(self, action):
        # Control vector (shape (1, ) in this case)
        u = np.zeros(1)

        masspole = self.unsquash(action[0], self.masspole_high, self.masspole_low)
        masscart = self.unsquash(action[1], self.masscart_high, self.masscart_low)
        length = self.unsquash(action[2], self.length_high, self.length_low)
        friction = self.unsquash(action[3], self.friction_high, self.friction_low) 
        g = -10

        s = 1

        gain = self.controller(masspole, masscart, length, g, friction, s)
        ...
        # u[t] = -Ky[t]
        u[:] = -np.dot(gain, self.state - self.goal_state)

        # Run simulation one time-step
        observation, reward, done, info = self.step_dynamic(u)
        info = {"x": x, "x_dot":x_dot, "theta":theta, "theta_dot":theta_dot, 'masspole': masspole, 'masscart':masscart, 'length':length, 'friction':friction, "control_action":u}

        return observation, reward, done, info

    def step_ideal(self, error=np.random.randn((4))):
        # Control vector (shape (1, ) in this case)
        u = np.zeros(1)

        masspole = self.masspole + error[0]
        masscart = self.masscart + error[1]
        length = self.length + error[2]
        friction = self.friction + error[3]
        g = -10
        s = 1
    ...
    def reset(self):
        ...
        self.state = np.array((-2, 0.0, np.pi-0.3, 0.1), dtype=np.float32)
        ...
        output = self.output(self.state, 0.0)
        self.time_step = 0
