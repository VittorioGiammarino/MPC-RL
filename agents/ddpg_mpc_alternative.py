...
class DDPGActor(nn.Module):
    def __init__(self, init_params, device, low=0.001, high=10):
        super().__init__()
        ...
        self.init_params = torch.tensor(init_params)
    ...
    def forward(self, obs, std):
        ...
        mu = self.params.to(self.device)*torch.ones((batch_size, self.params.shape[0])).to(self.device)
...
class DDPG_Agent:
    """DDPG algorithm."""
    def __init__(self, obs_dim, net_action_dim, ctrl_dim, ctrl_horizon_dim, action_range, params_range, init_params,
        ...
        self.actor = DDPGActor(init_params, device, params_range[0], params_range[1])

        # optimizers
        print(self.actor.params)
        self.actor_optimizer = torch.optim.Adam([self.actor.params],
                                                lr=actor_lr,
                                                betas=actor_betas)
